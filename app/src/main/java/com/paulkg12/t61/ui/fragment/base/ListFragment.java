package com.paulkg12.t61.ui.fragment.base;
...
//                                itemCount / getPagerSize() : (itemCount / getPagerSize()) + 1;
    ...
    //tt 这里进行了检测 滑动到最后一项的时候，进行 curPage+1， 然后 loadMore
    ...
    private void initScrollListener(){
        ...
        recyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                super.onScrolled(recyclerView, dx, dy);
                if(dy > 0){
                    mListScrollListener.onScrollUp();//tt1, the IssueActivity will do his job when I up, since he is the listener setted in
                } else {
                    mListScrollListener.onScrollDown();//tt2, the IssueActivity will do his job when I down, since he is the listener setted in
                }
            }
        });
    }
    //tt IssueActivity implements this interface, so IssueActivity he can
    //tt be set into this ListFragment's life, when ListFragment is scrollUP or Down
    //tt IssueActivity he can do tt1 and tt2 above
